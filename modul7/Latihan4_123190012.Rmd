---
title: "Latihan4_123190012"
author: "Afifrendra Rifqi Nugraha"
date: "11/7/2021"
output: pdf_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## D. Latihan


## 1. 
Gunakan as_tibble untuk mengkonversi tabel dataset “US murders” dalam bentuk tibble dan simpan dalam objek baru bernama ‘murders_tibble’.
```{r}
library(dslabs)
library(tidyverse)
library(tibble)
library(dplyr)
library(purrr)
data(murders)

as_tibble(murders)%>% class
```
```{r}
murders_tibble <- as_tibble(murders)
murders_tibble %>% class()
```

## 2. 
Gunakan fungsi group_by untuk mengkonversi dataset “US murders” menjadi sebuah tibble yang dikelompokkan berdasarkan ‘region’.
```{r}
as_tibble(murders) %>% group_by(region) 
```
```{r}
as_tibble(murders) %>% group_by(region) %>% class()
```

## 3. 
Tulis script tidyverse yang menghasilkan output yang sama dengan perintah berikut:
exp(mean(log(murders$population)))
Gunakan operator pipe sehingga setiap fungsi dapat dipanggil tanpa menambahkanargumen. Gunakan dot operator untuk mengakses populasi.
```{r}
murders$population %>% log %>% mean %>% exp
```

## 4. 
Gunakan map_df untuk membuat data frame yang terdiri dari tiga kolom: ‘n’, ‘s_n’, dan ‘s_n_2’. Kolom pertama harus berisi angka 1 hingga 100. Kolom kedua dan ketiga masingmasing harus berisi penjumlahan 1 hingga n, dimana n menyatakan jumlah baris.
```{r}


compute_s_n <- function(n){
x <- 1:n
tibble(n=n,s_n = sum(x),s_n_2 = sum(x))
}
n <- 1:100
Data <- map_df(n, compute_s_n)
Data
```